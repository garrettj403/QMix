...
_file_structure = {'DC IV curve':               'dciv/',
                   'Pumped IV curve':           'iv-curves/',
                   'IV + IF':                   'iv-if/',
                   'Impedance recovery':        'impedance/',
                   'Impedance recovery (sim)':  'impedance-sim/',
                   'IF noise':                  'if-noise/',
                   'Noise temperature':         'noise-temp/',
                   'IF spectrum':               'if-spectrum/',
                   'Overall performance':       'overall-results/',
                   'CSV data':                  'csv/'}
...
class DCData(object):
    ...
    def plot_all(self, fig_folder, sub_folder=None, **kw):  # pragma: no cover
        ...
        if sub_folder is None:
            sub_folder = _file_structure['DC IV curve']
...
class PumpedData(object):
    ...
    def plot_all(self, fig_folder, file_struc=None, **kw):  # pragma: no cover
        ...
        ``'Pumped IV curve'``, ``'IV + IF'``, ``'Impedance recovery'``, and
        ...
        if file_struc is None:
            iv_folder = _file_structure['Pumped IV curve']
            if_folder = _file_structure['IV + IF']
            zr_folder = _file_structure['Impedance recovery']
            zs_folder = _file_structure['Impedance recovery (sim)']
            ...
            iv_folder = file_struc['Pumped IV curve']
            if_folder = file_struc['IV + IF']
            zr_folder = file_struc['Impedance recovery']
            zs_folder = file_struc['Impedance recovery (sim)']
        ...
        zs_folder = os.path.join(fig_folder, zs_folder)
        ...
        if not os.path.exists(zs_folder):
            os.makedirs(zs_folder)
        ...
        fig2 = os.path.join(if_folder, f + '-iv-if.png')
        fig3 = os.path.join(zr_folder, f + '-err-surf.png')
        fig4 = os.path.join(zs_folder, f + '-sim.png')
